    ...
    std::map<std::string, std::shared_ptr<Tensor<float>>> tensor_maps[gpu_resource_group->size()];
    {
      ...
      // Create Embedding
      {
        for (unsigned int i = 1; i < j_layers_array.size(); i++) {
          ...
          data_reader->get_hashvalue(embedding.back()->get_hashvalue());
          data_reader->data_collector_->hash_tables_=embedding.back()->hash_tables_;
data_reader_eval->get_hashvalue(embedding.back()->get_hashvalue());
         data_reader_eval->data_collector_->hash_tables_=embedding.back()->hash_tables_;
          
          //data_reader->get_hashtable(embedding.back()->hash_tables_);
          for (unsigned int i = 0; i < gpu_resource_group->size(); i++) {
