...
__global__ void replace_insert_kernel(Table* table, const typename Table::key_type* const keys,
                               typename Table::mapped_type*  vals, size_t len,Table* table1,Table* table2,counter_type* d_counter,int* s_counter,typename Table::key_type* replacedkey,typename Table::key_type* replacedindex) {
  ...
  if (i < len) {
    ...
    auto it = table->replace_insert(kv, op,d_counter,table1->m_hashtbl_values,table2->m_hashtbl_values,s_counter,replacedkey,replacedindex);
    vals[i]=it->second;
...
__global__ void search_kernel(Table* table, const typename Table::key_type* const keys,
                              typename Table::mapped_type* const vals, size_t len, typename Table::key_type*  misskeys,int* counter) {
  const size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < len) {
    auto it = table->find(keys[i]);
    if(it == table->end() ){
...
__global__ void size_kernel(const Table* table, const size_t hash_capacity, int* table_size,
                            KeyType unused_key) {
...
__global__ void dump_kernel(KeyType* d_key, ValType* d_val, const Table* table, const size_t offset,
                            const size_t search_length, int* d_dump_counter,
                            KeyType unused_key) {
  ...
  if (i < search_length) {
    ...
    if (val.first != unused_key) {
      int local_index = atomicAdd(&block_acc, 1);
...
class HashTable {
 public:
  ...
  template <typename Table>
  void replace_insert(const KeyType* d_keys,   ValType* d_vals, size_t len, cudaStream_t stream,Table* table1,Table* table2,int* s_counter,KeyType *replacedkey,KeyType*replacedindex) {
    if (len == 0) {
    ...
    const int grid_size = (len - 1) / BLOCK_SIZE_ + 1;
    replace_insert_kernel<<<grid_size, BLOCK_SIZE_, 0, stream>>>(table_, d_keys, d_vals, len,table1,table2,d_counter_,s_counter,replacedkey,replacedindex);
  }
  /**
   ...
   * @param stream the cuda stream for this operation.
   */
  void get(const KeyType* d_keys, ValType* d_vals, size_t len, cudaStream_t stream,KeyType* misskey,int* counter) const {
    if (len == 0) {
    ...
    const int grid_size = (len - 1) / BLOCK_SIZE_ + 1;
    search_kernel<<<grid_size, BLOCK_SIZE_, 0, stream>>>(table_, d_keys, d_vals, len,misskey,counter);
  }
  /**
   ...
   * @param stream the cuda stream for this operation.
   */
  int get_size(cudaStream_t stream) const {    
    ...
    CK_CUDA_THROW_(cudaMalloc((void**)&d_table_size, sizeof(int)));
    CK_CUDA_THROW_(cudaMemset(d_table_size, 0, sizeof(int)));
    size_kernel<<<grid_size, BLOCK_SIZE_, 0, stream>>>(table_, hash_capacity, d_table_size,
                                                       empty_key);
    CK_CUDA_THROW_(
        cudaMemcpyAsync(&table_size, d_table_size, sizeof(int), cudaMemcpyDeviceToHost, stream));
  ...
  void dump(KeyType* d_key, ValType* d_val, const size_t offset, const size_t search_length,
            int* d_dump_counter, cudaStream_t stream) {
    // Before we call the kernel, set the global counter to 0
    CK_CUDA_THROW_(cudaMemset(d_dump_counter, 0, sizeof(int)));
