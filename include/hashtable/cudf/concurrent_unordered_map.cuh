...
template <typename aggregation_type, typename counter_type, class comparison_type = key_equal,
            typename hash_value_type = typename Hasher::result_type>
  __forceinline__ __device__ iterator replace_insert(const value_type& x, aggregation_type op,
  counter_type* value_counter, value_type* hashtbl_values1,  value_type* hashtbl_values2,int* s_counter,key_type*replacedkey,mapped_type*replacedindex,
                                             comparison_type keys_equal = key_equal(),
                                             bool precomputed_hash = false,
                                             hash_value_type precomputed_hash_value = 0) {
    const size_type hashtbl_size = m_hashtbl_size;
    ...
    while (false == insert_success) {
      ...
      if (keys_equal(unused_key, old_key) ) {
          ...
          // printf("unused!!  %llu  %llu  ",old_key,insert_key);
        insert_success = true;
      }else if(keys_equal(insert_key, old_key)){
        //  printf("have used!!  %llu  %llu  ",old_key,insert_key);
        insert_success = true;
      }
      else {
        bool success1=true,success2=true;
         hash_value_type hash_value1=m_hf(old_key);
          size_type current_index1 = hash_value1 % hashtbl_size;
        size_type current_index2 = hash_value1 % hashtbl_size;
         value_type* current_hash_bucket1 = &(hashtbl_values1[current_index1]);
    value_type* current_hash_bucket2 = &(hashtbl_values2[current_index2]);

        while(!keys_equal(unused_key, current_hash_bucket1->first)){
          if(keys_equal(old_key,current_hash_bucket1->first)){
            success1=false;
            break;
          }
           current_index1 = (current_index1 + 1) % hashtbl_size;
      current_hash_bucket1 = &(hashtbl_values1[current_index1]);
        }
        if(success1==true){
          while(!keys_equal(unused_key, current_hash_bucket2->first)){
          if(keys_equal(old_key,current_hash_bucket2->first)){
            success2=false;
            break;
          }
           current_index2 = (current_index2 + 1) % hashtbl_size;
      current_hash_bucket2 = &(hashtbl_values2[current_index2]);
        }
        }
        if(success1&&success2){
           int index=(int)(atomicAdd(s_counter, 1));
         replacedkey[index]=old_key;
         replacedindex[index]=existing_value;
         const key_type old_key1=atomicCAS(&existing_key, old_key, insert_key);
         
        
         if(keys_equal(old_key1, old_key)){
            insert_success = true;
            // printf("replaced!!  %llu  %llu  ",old_key,insert_key);
         }
        }
      }
...
#ifdef __CUDA_ARCH__
          if ( std::numeric_limits<key_type>::is_integer &&hashtbl_size
          {
...
int*>(tmp_it), unused, value ); if ( old_val == unused ) { it = tmp_it;
              ...
              if ( m_equal( unused_key, old_key ) ) {
                  (m_hashtbl_values+hash_tbl_idx)->second = x.second;m_hashtbl_size
              else if ( count_collisions )
              {
                  atomicAdd( &m_collisions, 1 );
              }
...
template <typename aggregation_type, typename counter_type, class comparison_type = key_equal,
  ...
  __forceinline__ __device__ iterator get_insert(const key_type& k, aggregation_type op,
    ...
    while (false == insert_success) {
      ...
      // operation, it is safe to perform the operation when the existing_value still
      // has its initial value
      // TODO: Use template specialization to make use of native atobreak
       if (keys_equal(unused_key, old_key)) {
        existing_value = (mapped_type)(atomicAdd(value_counter, 1));
        break;
...
value_type* getbuket(cudaStream_t stream = 0){
