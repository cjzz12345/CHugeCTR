...
class LocalizedSlotSparseEmbeddingHash : public Embedding<TypeHashKey> {
 ...
 public:
  ...
  LocalizedSlotSparseEmbeddingHash(const Tensors<TypeHashKey> &row_offsets_tensors,
      ...
      // std::vector<std::unique_ptr<Base::NvHashTable>> get_table() override;  
...
// template <typename TypeHashKey>
// std::vector<std::unique_ptr< Base::NvHashTable>> LocalizedSlotSparseEmbeddingHash<TypeHashKey>::get_table(){
//   return Base::hash_tables_;
// }
template <typename TypeHashKey>
Tensors<float>  LocalizedSlotSparseEmbeddingHash<TypeHashKey>:: get_hashvalue(){
  ...
  try {
    ...
    for (int id = 0; id < local_gpu_count_; id++) {
      ...
      // construct HashTable object: used to store hash table <key, value_index>
       Base::hash_tables_.emplace_back(new NvHashTable(max_vocabulary_size_per_gpu_));
...
void LocalizedSlotSparseEmbeddingHash<TypeHashKey>::forward() {
  ...
  functors_.forward(embedding_params_.batch_size, slot_num_per_gpu_,
                    ...
                    Base::row_offsets_tensors_, Base::value_tensors_,  Base::hash_tables_,
