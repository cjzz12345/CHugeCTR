...
class SparseEmbeddingHashFunctors {
 ...
 public:
  ...
  void forward(
      ...
      const std::shared_ptr<GPUResourceGroup> &device_resources, const CudaDeviceContext &context,int &round) {
    int local_gpu_count = device_resources->size();
    ...
    for (int id = 0; id < local_gpu_count; id++) {
      ...
      const auto &hash_table = hash_tables[id*3].get();
      ...
      //  const auto &hash_table1 = hash_tables[id*3+1].get();
      //  const auto &hash_table2 = hash_tables[id*3+2].get();
      try {
        // get hash_value_index from hash_table by hash_key

        size_t num;
        int* counter,* h_counter,cc;
        CK_CUDA_THROW_(cudaMemcpyAsync(&num, &row_offset[batch_size * slot_num],
          ...
          //     hash_table1->insert(hash_key, hash_value_index, num, stream);
          ...
          //     hash_table2->insert(hash_key, hash_value_index, num, stream);
          //   //  std::cout<<"3  2"<<hash_table2->get_size(stream)<<std::endl;
          //   }
          //   round=(round+1)%2;
            
  
//             TypeHashKey *missingkey,*h_missingkey,* h_hashkey;
//              cudaMalloc(&missingkey, 512* sizeof(TypeHashKey));
//              cudaMallocHost(&h_missingkey, 512* sizeof(TypeHashKey));
//              cudaMalloc(&counter, sizeof(int));
//               cudaMallocHost(&h_counter,  sizeof(int));
//                cudaMallocHost(&h_hashkey, 512* sizeof(TypeHashKey));
...
//             std::unique_ptr<int *[]> d_dump_counter(new int *[local_gpu_count]);
//  cudaMalloc(&d_hash_table_key[id], 208* sizeof(TypeHashKey));
//       cudaMalloc(&d_hash_table_value_index[id], 208* sizeof(TypeHashValueIndex));
//                   cudaMalloc(&d_dump_counter[id], 208* sizeof(int));
//             * h_counter=0;
...
// cc=hash_table->get_size((*device_resources)[id]->get_stream());
// hash_table->dump(d_hash_table_key[id], d_hash_table_value_index[id], 0,
//                             150, d_dump_counter[id],
//                             (*device_resources)[id]->get_stream());
//   CK_CUDA_THROW_(cudaMemcpyAsync(h_hashkey, d_hash_table_key[id],
//                                       150*sizeof(TypeHashKey), cudaMemcpyDeviceToHost,
//                                      (*device_resources)[id]->get_stream()));

//                                       std::cout <<"    before replace insert part                     "<<cc<<std::endl<<std::endl;    
//         for(int i=0;i<150 ;i++)
//   {
//     std::cout <<"    before replace insert                    "<<i<<"       " <<h_hashkey[i];
//   }                          
// std::cout<<std::endl<<std::endl;    
//  CK_CUDA_THROW_(cudaMemcpyAsync(h_hashkey, hash_key,
//                                       104*sizeof(TypeHashKey), cudaMemcpyDeviceToHost,
//                                      (*device_resources)[id]->get_stream()));
...
// std::cout<<std::endl<<std::endl;    
        ...
        //                              sizeof(int), cudaMemcpyHostToDevice,
        //                              stream));
        // hash_table->get(hash_key, hash_value_index, num, stream,missingkey,counter);
        // CK_CUDA_THROW_(cudaMemcpyAsync(h_counter, counter,
        //                              sizeof(int), cudaMemcpyDeviceToHost,
        //                              (*device_resources)[id]->get_stream()));
        //                              cudaStreamSynchronize((*device_resources)[id]->get_stream());    
        // hash_table->replace_insert(missingkey, hash_value_index, *h_counter, stream,hash_table1->table_,hash_table2->table_,s_counter);
...
//                                      stream));
//           std::cout<<"s_counter      "<<*s_counter1<<std::endl<<std::endl;  
//   CK_CUDA_THROW_(cudaMemcpyAsync(h_missingkey, missingkey,
//                                       512*sizeof(TypeHashKey), cudaMemcpyDeviceToHost,
//                                      (*device_resources)[id]->get_stream()));
//                                      cudaStreamSynchronize((*device_resources)[id]->get_stream());    
  
//        std::cout <<"    missingkeys part          111           "<<std::endl<<std::endl;                                     
...
//                                      (*device_resources)[id]->get_stream()));
                                      //    cudaStreamSynchronize((*device_resources)[id]->get_stream());    
                                      //  std::cout <<"    before   get insert                   "<<cc<<std::endl<<std::endl;                           
//         for(int i=0;i<150 ;i++)
//   {
...
//                                      sizeof(int), cudaMemcpyHostToDevice,
//                                      (*device_resources)[id]->get_stream()));
// hash_table->get(hash_key, hash_value_index, num, (*device_resources)[id]->get_stream(),missingkey,counter);
//         CK_CUDA_THROW_(cudaMemcpyAsync(h_counter, counter,
//                                      sizeof(int), cudaMemcpyDeviceToHost,
//                                      (*device_resources)[id]->get_stream()));
//                                      cudaStreamSynchronize((*device_resources)[id]->get_stream());    
...
//                                      (*device_resources)[id]->get_stream()));
//                                          cudaStreamSynchronize((*device_resources)[id]->get_stream());    
                                      //  std::cout <<"     after get insert                    "<<cc<<std::endl<<std::endl;                           
//         for(int i=0;i<150 ;i++)
//   {
//     std::cout <<"     after replace insert                    "<<i<<"       " <<h_hashkey[i];
//   }                          
// std::cout<<std::endl<<std::endl;   
  ...
  void upload_params_to_device(
      ...
      std::vector<std::shared_ptr<  HashTable<TypeHashKey, TypeHashValueIndex, std::numeric_limits<TypeHashKey>::max()>>> &hash_tables,
      const std::shared_ptr<GPUResourceGroup> &device_resources, const CudaDeviceContext &context) {
